...
class JumpServer(object):
    ...
    def display_server(self,group_id,search=False,search_value=None):
        ...
        for i,s in enumerate(server_query_set):
            self.server_dic[i] = s
            x.add_row([i,s.server_name,s.ipaddress,s.ssh_port])
        print x
    ...
    def deal_audit_log(self,cmd,s):
        msg = 'server name : %s , ip : %s , user : %s , cmd : %s ' % (s.server_name,s.ipaddress,self.username,cmd)
        self.models.JumpServerAudit.objects.create(username=self.username,content=msg)

    def login(self,s):
        try:
            self.unsupport_cmd_list = ['rz','sz']
            tran = paramiko.Transport((s.ipaddress, s.ssh_port,))
            tran.connect(username='root', password=s.root_pwd)
            # 打开一个通道
            chan = tran.open_session()
            # 获取一个终端
            chan.get_pty(term='xterm', width=200, height=240)
            # 激活器
            chan.invoke_shell()
