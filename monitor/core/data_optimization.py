    ...
    def get_data_slice(self,lastest_data_key,optimization_interval):
        ...
        for item in all_real_data:
            ...
            if len(data) ==2:
                ...
                print last_save_time,time.time(),optimization_interval
    ...
    def process_and_save(self):
        ...
        if self.data['status'] ==0:# service data is valid
            with self.rabbit_con_pool.acquire() as cxn:
                cxn.channel.queue_declare(queue=settings.RABBIT_CHECK_SERVICE_ALIVE_QUEUE, durable=True)
                cxn.channel.basic_publish(
                    body=json.dumps({'server_id':int(self.client_id),
                          'service_name':self.service_name
                          }),
                    exchange='',
                    routing_key=settings.RABBIT_CHECK_SERVICE_ALIVE_QUEUE,
